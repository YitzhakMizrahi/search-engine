...
function AutoSearchResults({ results, search, input }) {
  ...
  return (
    <div className="w-full rounded-3xl focus-within:shadow-lg rounded-t-none border border-gray-200 py-3 max-w-md shadow-lg sm:max-w-xl lg:max-w-2xl items-center cursor-pointer">
      {results.data?.items?.map((result) => (
        <div key={result.link} className="pt-2 pb-2 px-6 hover:bg-gray-50">
